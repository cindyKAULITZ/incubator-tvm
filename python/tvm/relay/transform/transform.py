...
# hhliao
def Conv2dToImg2Col(weight_name, weight_shape):
    """
    Rewrite qualified ```nn.dense operation``` to ```nn.img2col_conv2d```
    This pass is used in ```data_dep_optimization.bsr_dense```
    Parameters of this pass is generated by ```analysis.img2col_conv2d.process_params```
    ...
    -------
    ret : tvm.transform.Pass
        The registered Conv2dToImg2Col pass.
    """
    return _ffi_api.Conv2dToImg2Col(weight_name, weight_shape)
